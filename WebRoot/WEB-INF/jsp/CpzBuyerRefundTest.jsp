...
<script
	...
	<script>
		...
		$(document).on('ready', function() {
			...
			var addInp="?refundId=&userId=&orderNo=&refundReason=&isConsultSeller=&refundStyle=&orderMoney=&applyMoney=&applyTime=&refundTime=";
			$("#addIn").val(addInp);
			var updateInp="?refundId=&userId=&orderNo=&refundReason=&isConsultSeller=&refundStyle=&orderMoney=&applyMoney=&applyTime=&refundTime=";
			$("#updateIn").val(updateInp);
			var deleteInp="?refundId=";
...
var requesturl='http://localhost:8080/'+projectName+'cpzbuyerrefund/query.do'+searchInp;
			
				$.ajax({
					type:'POST',
					url:'cpzbuyerrefund/query.do'+searchInp,
					data:{
					},
...
var pathName=window.document.location.pathname;
var projectName=pathName.substring(1,pathName.substr(1).indexOf('/')+1);
var requesturl='http://localhost:8080/'+projectName+'cpzbuyerrefund/insert.do'+addInp;
			
				$.ajax({
					type:'POST',
					url:'cpzbuyerrefund/insert.do'+addInp,
					data:{
					},
					success:function(result){
							var returndata = result.returnData;
...
var pathName=window.document.location.pathname;
var projectName=pathName.substring(1,pathName.substr(1).indexOf('/')+1);
var requesturl='http://localhost:8080/'+projectName+'/cpzbuyerrefund/update.do'+updateInp;
			
				$.ajax({
					type:'POST',
					url:'cpzbuyerrefund/update.do'+updateInp,
					data:{
					},
					success:function(result){
							var returndata = result.returnData;
...
var pathName=window.document.location.pathname;
var projectName=pathName.substring(1,pathName.substr(1).indexOf('/')+1);
var requesturl='http://localhost:8080/'+projectName+'/cpzbuyerrefund/delete.do'+deleteInp;
			
				$.ajax({
					type:'POST',
					url:'cpzbuyerrefund/delete.do'+deleteInp,
					data:{
					},
					success:function(result){
							var returndata = result.returnData;
