...
<script
	...
	<script>
		...
		$(document).on('ready', function() {
			...
			var addInp="?orderLogId=&dealTime=&dealType=&dealContent=&dealPerson=&lonValue=&latValue=";
			$("#addIn").val(addInp);
			var updateInp="?orderLogId=&dealTime=&dealType=&dealContent=&dealPerson=&lonValue=&latValue=";
			$("#updateIn").val(updateInp);
			var deleteInp="?orderLogId=";
...
var requesturl='http://localhost:8080/'+projectName+'cpzbuyerlog/query.do'+searchInp;
			
				$.ajax({
					type:'POST',
					url:'cpzbuyerlog/query.do'+searchInp,
					data:{
					},
...
var pathName=window.document.location.pathname;
var projectName=pathName.substring(1,pathName.substr(1).indexOf('/')+1);
var requesturl='http://localhost:8080/'+projectName+'cpzbuyerlog/insert.do'+addInp;
			
				$.ajax({
					type:'POST',
					url:'cpzbuyerlog/insert.do'+addInp,
					data:{
					},
					success:function(result){
							var returndata = result.returnData;
...
var pathName=window.document.location.pathname;
var projectName=pathName.substring(1,pathName.substr(1).indexOf('/')+1);
var requesturl='http://localhost:8080/'+projectName+'/cpzbuyerlog/update.do'+updateInp;
			
				$.ajax({
					type:'POST',
					url:'cpzbuyerlog/update.do'+updateInp,
					data:{
					},
					success:function(result){
							var returndata = result.returnData;
...
var pathName=window.document.location.pathname;
var projectName=pathName.substring(1,pathName.substr(1).indexOf('/')+1);
var requesturl='http://localhost:8080/'+projectName+'/cpzbuyerlog/delete.do'+deleteInp;
			
				$.ajax({
					type:'POST',
					url:'cpzbuyerlog/delete.do'+deleteInp,
					data:{
					},
					success:function(result){
							var returndata = result.returnData;
