...
<script
	...
	<script>
		...
		$(document).on('ready', function() {
			...
			var addInp="?messageId=&messageDetail=&messageType=&readMessageFlag=&systemNo=&messageChannel=&messageLinkType=&messageLinkPara=";
			$("#addIn").val(addInp);
			var updateInp="?messageId=&messageDetail=&messageType=&readMessageFlag=&systemNo=&messageChannel=&messageLinkType=&messageLinkPara=";
			$("#updateIn").val(updateInp);
			var deleteInp="?messageId=";
...
var requesturl='http://localhost:8080/'+projectName+'cpzmessage/query.do'+searchInp;
			
				$.ajax({
					type:'POST',
					url:'cpzmessage/query.do'+searchInp,
					data:{
					},
...
var pathName=window.document.location.pathname;
var projectName=pathName.substring(1,pathName.substr(1).indexOf('/')+1);
var requesturl='http://localhost:8080/'+projectName+'cpzmessage/insert.do'+addInp;
			
				$.ajax({
					type:'POST',
					url:'cpzmessage/insert.do'+addInp,
					data:{
					},
					success:function(result){
							var returndata = result.returnData;
...
var pathName=window.document.location.pathname;
var projectName=pathName.substring(1,pathName.substr(1).indexOf('/')+1);
var requesturl='http://localhost:8080/'+projectName+'/cpzmessage/update.do'+updateInp;
			
				$.ajax({
					type:'POST',
					url:'cpzmessage/update.do'+updateInp,
					data:{
					},
					success:function(result){
							var returndata = result.returnData;
...
var pathName=window.document.location.pathname;
var projectName=pathName.substring(1,pathName.substr(1).indexOf('/')+1);
var requesturl='http://localhost:8080/'+projectName+'/cpzmessage/delete.do'+deleteInp;
			
				$.ajax({
					type:'POST',
					url:'cpzmessage/delete.do'+deleteInp,
					data:{
					},
					success:function(result){
							var returndata = result.returnData;
